...
| # | Note | State |
|---|---|---|
| 41 | "The setup here should happen from the plugin itself… probably an action button." | ✅ **First-run wizard.** Until now the plugin was useless until someone ran `git clone` in a terminal — the single biggest barrier for the audience it is for. An empty vault now offers *Set up your Azure DevOps wiki*: paste the address from the portal's **Clone wiki** dialog and it does the rest, also available as the command **Set up a wiki in this vault**. `git clone` cannot be used (the vault already holds `.obsidian/`, and clone refuses a non-empty directory — asserted in a test so the workaround does not become cargo cult), so it is `init` + `remote add` + `fetch` + `checkout -t`, with the branch discovered from the server via `ls-remote` and `chooseCloneBranch`. Organization, project and wiki name are parsed out of the pasted URL rather than asked for, and a password in it is stripped so no PAT is written into `.git/config`. Refuses, without creating `.git`, when the vault is already a clone, sits inside another repository, or holds notes a checkout could overwrite. **Untested against real Azure DevOps auth** — if Git Credential Manager ever falls back to a console prompt there is no TTY, and the fetch will sit until the 120 s timeout |
| 40 | "I did not get the error. When clicked on get updates, it says updated, but nothing is pulled." | ✅ The vault was a scratch folder created *inside* the plugin's own checkout, and `git` searches upwards — so `isRepo()` was true, the branch was `main` with an upstream, and every guard rail passed: Get updates fetched **the plugin's repository** and honestly reported success. Note 39 removed the accident that had been covering this (`main` was not adopted before, so `wrongBranch` fired). `checkGuardRails` now requires the vault to be the repository **root**, which is the real invariant — Publish was the dangerous direction, since it would have committed the vault's subtree and pushed it to that repository. Asked of git via `--show-prefix`, because comparing git's path with Obsidian's cannot work on Windows (see CLAUDE.md: 8.3 short names) |
